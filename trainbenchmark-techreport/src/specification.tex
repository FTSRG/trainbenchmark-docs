...
\begin{itemize}
  ...
  \item \concept{Automated model repair scenairo} (\textsf{Repair}):
...
\begin{figure}
        ...
        \label{fig:trainbenchmark-transformations-repair}
\end{figure}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item \emph{PosLength:} Randomly selected \emph{segments'} \emph{length} attribute is updated to 0, which means that an error is injected (\figref{fig:trainbenchmark-transformation-user-poslength}).\footnote{In EMF this means that an \texttt{int} attribute is set (updated), while in other representations (e.g.\ RDF databases) first the assertion about the old value is removed and the assertion stating the new value of the length is inserted.}
    
    \item \emph{RouteSensor:} The \emph{routeDefinition} edges between the randomly selected \emph{routes} and their \emph{first} connected \emph{sensor} are removed (at most one edge for each route, see~\figref{fig:trainbenchmark-transformation-user-routesensor}).
    
    \item \emph{SignalNeighbor:} Errors are introduced by disconnecting the \emph{entry} edge of the selected \emph{routes} (\figref{fig:trainbenchmark-transformation-user-signalneighbor}). (According to the metamodel, a \emph{route} may only have 0 or 1 \emph{entry} edges).

    \item \emph{SwitchSensor:} Errors are injected by randomly selecting \emph{switches} and deleting all their edges to \emph{sensors}. If the chosen \emph{switch} was invalid, it did not have such an edge, so no edges are deleted and the \emph{switch} stays invalid. If the chosen \emph{switch} was valid, it will become invalid (\figref{fig:trainbenchmark-transformation-user-switchsensor}).
...
\begin{figure}
        ...
        \begin{subfigure}[b]{\textwidth}
        		\centering
                \includegraphics[scale=0.4]{figures/trainbenchmark-transformation-repair-poslength}
                \caption{\textsf{PosLength}}
                \label{fig:trainbenchmark-transformation-repair-poslength}
        \end{subfigure}
        ~
        \begin{subfigure}[b]{\textwidth}
                \centering
                \includegraphics[scale=0.4]{figures/trainbenchmark-transformation-repair-routesensor}
                \caption{\textsf{RouteSensor}}
                \label{fig:trainbenchmark-transformation-repair-routesensor}
        \end{subfigure}
        ~
        \begin{subfigure}[b]{\textwidth}
                \centering
                \includegraphics[scale=0.4]{figures/trainbenchmark-transformation-repair-signalneighbor}
                \caption{\textsf{SignalNeighbor}}
                \label{fig:trainbenchmark-transformation-repair-signalneighbor}
        \end{subfigure}
        ~
        \begin{subfigure}[b]{\textwidth}
                \centering
                \includegraphics[scale=0.4]{figures/trainbenchmark-transformation-repair-switchsensor}
                \caption{\textsf{SwitchSensor}}
                \label{fig:trainbenchmark-transformation-repair-switchsensor}
        \end{subfigure}
...
\end{figure}
  ...
  \begin{itemize}
    ...
    \item \emph{PosLength:} Random elements are selected from the set of invalid \emph{segments} and their values are updated to $- \mathit{oldValue} + 1$ (\figref{fig:trainbenchmark-transformation-repair-poslength}).
    
    \item \emph{RouteSensor:} Randomly selected invalid \emph{sensors} are disconnected from the \emph{switch}, which means that the constraint will no longer apply (\figref{fig:trainbenchmark-transformation-repair-routesensor}).
    
    \item  \emph{SignalNeighbor:} Disconnect \emph{exit} references of randomly selected invalid \emph{routes}, resulting in a structure where the constraint must not hold for the actual route (\figref{fig:trainbenchmark-transformation-repair-signalneighbor}).
