...
For the benchmark scenario we opted for a simple execution schema that represents a batch-validation scenario. This is the same, as the first two phase of the original benchmark. See \figref{fig:phases}) and \secref{sec:phases} for description of the \emph{read} and \emph{check} phases. 
...
queries tasks, independently of the actual strategy or technological solution
...
The metrics discussed in the following, tipify instance models, queries or their combination, without characterizing a unique property of a specific graph/query description language or environment. Most of these metrics have previously been defined by other sources, while others are newly proposed in this paper.
...
graph based visualisation of these pattern structures is shown in Fig.
\ref{fig:aselocals}, where the node drawn with empty circle represents the
...
(returning one parameter) is shown in Fig. \ref{fig:aseparams}, where the
...
schema in Fig. \ref{fig:asechecks}, but each one is extended with an increasing
...
on them. Table~\ref{tab:queryonlymetrics} shows the results of the evaluation:
