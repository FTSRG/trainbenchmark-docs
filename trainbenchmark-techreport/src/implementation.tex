...
\begin{enumerate}
  ...
  \item Merge measured data into results.txt. The file contains one header line and multiple data record belonging to (optionally) multiple series.
  \item Modify \textsf{configs.R} to introduce new tool (specify tool name in the results.txt and on the diagrams), specify line color and type of the tool, and in the \textsf{drawtools} variable enumerate tools to be plotted.
  \item Execute \textsf{knitIt.sh}
  \item Inspect the generated \textsf{plotLines.html} and \textsf{plotLines-user.html} reports.
  \item If the report should be zipped, execute \textsf{packHTML.sh} that generates \textsf{tbReport.zip}.
\end{enumerate}
